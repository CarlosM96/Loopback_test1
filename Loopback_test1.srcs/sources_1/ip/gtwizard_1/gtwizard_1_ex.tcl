#-------------------------------------------------------------
# Generated Example Tcl script for IP 'gtwizard_1' (xilinx.com:ip:gtwizard:3.6)
#-------------------------------------------------------------

# Declare source IP directory
set srcIpDir "/home/daphnelme/Desktop/fullModeDAPHNE/Loopback_test1/Loopback_test1.srcs/sources_1/ip/gtwizard_1"

# Create project
puts "INFO: \[open_example_project\] Creating new example project..."
create_project -name gtwizard_1_ex -force
...
import_ip -files [list [file join $srcIpDir gtwizard_1.xci]] -name gtwizard_1
reset_target {open_example} [get_ips gtwizard_1]
...
generate_target -quiet [_filter_supported_targets {instantiation_template synthesis simulation implementation shared_logic} [get_ips gtwizard_1]] [get_ips gtwizard_1]
...
add_files -scan_for_includes -quiet -fileset [current_fileset] \
  [list [file join $srcIpDir gtwizard_1/example_design/gtwizard_1_exdes.v]] \
  [list [file join $srcIpDir gtwizard_1/example_design/gtwizard_1_gt_frame_check.v]] \
  [list [file join $srcIpDir gtwizard_1/example_design/gtwizard_1_gt_frame_gen.v]]
...
add_files -quiet -fileset [current_fileset] \
  [list [file join $srcIpDir gtwizard_1/example_design/gt_rom_init_tx.dat]]
set_property USED_IN_SIMULATION false [get_files [list [file join $srcIpDir gtwizard_1/example_design/gt_rom_init_tx.dat]]]
...
add_files -quiet -fileset [current_fileset -constrset] \
  [list [file join $srcIpDir gtwizard_1/example_design/gtwizard_1_exdes.xdc]]
...
add_files -quiet -scan_for_includes -fileset [current_fileset -simset] \
  [list [file join $srcIpDir gtwizard_1/simulation/gtwizard_1_tb.v]]
set_property USED_IN_SYNTHESIS false [get_files [list [file join $srcIpDir gtwizard_1/simulation/gtwizard_1_tb.v]]]
...
add_files -quiet -fileset [current_fileset -simset] \
  [list [file join $srcIpDir gtwizard_1/simulation/functional/gt_rom_init_tx.dat]]
set_property USED_IN_SYNTHESIS false [get_files [list [file join $srcIpDir gtwizard_1/simulation/functional/gt_rom_init_tx.dat]]]
...
if {[catch {source [file join $srcIpDir gtwizard_1/tcl/xci.tcl]} errMsg]} {
  puts "ERROR: \[open_example_project\] Open Example Project failed: Error encountered while sourcing custom IP example design script."
  puts "$errMsg"
  error "ERROR: see log file for details."
  incr returnCode
}
if {[catch {source [file join $srcIpDir gtwizard_1/tcl/gtwizard_1_partner.tcl]} errMsg]} {
  puts "ERROR: \[open_example_project\] Open Example Project failed: Error encountered while sourcing custom IP example design script."
  puts "$errMsg"
  error "ERROR: see log file for details."
  incr returnCode
